  ...
  <div>
    ...
    <div class="container">
      ...
      >
        ...
        <el-table-column prop="consultant" label="咨询人" width="55" align="center"></el-table-column>
        <el-table-column prop="number" label="电话号码"></el-table-column>
        <el-table-column prop="time" label="杰电话时间"></el-table-column>
...
export default {
  ...
  created() {
    const{code,message,date} = this.$api.number.getAllNumber(this.pageIndex,this.pageSize)
    if (code === 200){
      this.setNumberInfo(date)
      this.$message.success(message)
    }
  },
  methods: {
    ...mapActions('number',['setNumberInfo']),
    // 获取 easy-mock 的模拟数据
    // 触发搜索按钮
    handleSearch() {
